---
name: 智能查询系统设计规划
overview: 基于 PRD 文档，设计智能查询系统的完整项目结构、模块划分、核心数据流和分阶段实施计划。采用 PostgreSQL 演示数据库（电商场景）、OpenAI 兼容 API（gpt-5.2-chat via yunwu.ai 中转）、ChromaDB 内置 Embedding。
todos:
  - id: p1-backend-scaffold
    content: "Phase 1.1: 后端脚手架 — 初始化 FastAPI 项目结构、requirements.txt、config.py、main.py (CORS+路由)、health 接口"
    status: completed
  - id: p1-frontend-scaffold
    content: "Phase 1.2: 前端脚手架 — 初始化 React (Vite) + TypeScript 项目、安装 Tailwind/ECharts 等依赖、空壳 App.tsx"
    status: completed
  - id: p1-database-docker
    content: "Phase 1.3: 数据库与容器化 — 编写 init.sql 电商演示数据、docker-compose.yml (PG)、.env.example"
    status: completed
  - id: p1-run-test
    content: "Phase 1.4: 验证运行 — 后端 uvicorn 启动 + GET /health 返回 200；前端 npm run dev 页面可访问；PG 容器启动可连接"
    status: completed
  - id: p2-layout
    content: "Phase 2.1: 前端布局框架 — Header (YIWUSONG Logo + 标题 + 状态灯 + 返回首页) + Sidebar (查询历史) + MainContent 左右分栏布局"
    status: completed
  - id: p2-query-input
    content: "Phase 2.2: QueryInput 组件 — 底部固定输入框、Enter 发送、Shift+Enter 换行、禁用态、推荐问题"
    status: completed
  - id: p2-result-panel
    content: "Phase 2.3: 结果展示组件 — ThinkingDisplay (打字机效果) + SqlPreview (语法高亮+复制) + DataTable (查询结果表格) + EChartsRenderer (声明式渲染) + ErrorDisplay"
    status: completed
  - id: p2-sse-hook
    content: "Phase 2.4: SSE Hook + 状态流转 — useSSE.ts (fetch+ReadableStream)、UI 状态机 (Idle→Thinking→ShowSQL→ShowChart)、骨架屏"
    status: completed
  - id: p2-responsive-theme
    content: "Phase 2.5: 响应式 + 主题 — 三档断点适配、深色主题、动效过渡"
    status: completed
  - id: p3-schema-rag
    content: "Phase 3.1: Schema RAG 接口 — schema_extractor (PG元数据提取+注释+外键) + embedder (ChromaDB 内置 all-MiniLM-L6-v2) + retriever (语义 Top-K)"
    status: completed
  - id: p3-llm-engine
    content: "Phase 3.2: LLM Engine 接口 — OpenAI 兼容 API 调用 (gpt-5.2-chat via yunwu.ai)、流式输出、JSON 结构化解析（支持 ```json 代码块提取）"
    status: completed
  - id: p3-security
    content: "Phase 3.3: 安全层接口 — SQL 防火墙 (sqlglot AST 校验, 12 项 pytest 全通过) + query_limiter (自动LIMIT+超时+速率限制) + 只读连接池"
    status: completed
  - id: p3-query-api
    content: "Phase 3.4: 核心查询接口 — POST /api/query SSE + 状态机 (SchemaRetrieval→LLMGeneration→SQLValidation→SQLExecution→ResultStreaming) + POST /api/query/mock (4场景智能匹配)"
    status: completed
  - id: p3-schema-api
    content: "Phase 3.5: Schema 管理接口 — GET /api/schema/status + POST /api/schema/refresh + GET /api/schema/tables"
    status: completed
  - id: p4-integration
    content: "Phase 4.1: 前后端联调 — SSE Hook 对接后端全部事件类型 (state/thought/sql/data/viz_config/error/done)、DB 健康检测自动降级到 Mock"
    status: completed
  - id: p4-e2e-test
    content: "Phase 4.2: 端到端测试 — 前端代理验证、Mock SSE 全流程、curl 独立测试"
    status: completed
  - id: p4-polish
    content: "Phase 4.3: 优化收尾 — Docker Compose 三服务编排 (postgres/backend/frontend)、Dockerfile、nginx SSE 代理、Header 返回首页"
    status: completed
isProject: false
---
...
sequenceDiagram
    ...
    participant FW as SQL防火墙
    participant DB as PostgreSQL

    User->>FE: 输入自然语言问题
    FE->>FE: 检查 /health (DB状态)
    alt DB 可用
        FE->>BE: POST /api/query (SSE)
    else DB 不可用
        FE->>BE: POST /api/query/mock (降级)
    end
    BE->>RAG: 语义检索相关Schema
    RAG-->>BE: Top-K表结构
    BE->>SM: 初始化状态机
    SM->>LLM: System Prompt + Schema + 用户问题
    LLM-->>SM: stream: thinking (逐 token)
    SM-->>BE: event:thought (delta)
    BE-->>FE: SSE event:thought
    LLM-->>SM: 完整 JSON (thinking + sql + echarts_option)
    SM-->>BE: event:thought (done)
    SM->>FW: SQL AST校验 + 自动 LIMIT
    alt 校验通过
        FW-->>SM: PASS (安全SQL)
        SM-->>BE: event:sql
        BE-->>FE: SSE event:sql
        BE->>DB: 执行只读SQL (带超时)
        DB-->>BE: 查询结果
        SM-->>BE: event:data (列名+行数据+耗时)
        BE-->>FE: SSE event:data
        SM-->>BE: event:viz_config (用实际数据填充 ECharts option)
...
```mermaid
stateDiagram-v2
    [*] --> SchemaRetrieval: 用户提问
    SchemaRetrieval --> LLMGeneration: 获得Top-K Schema上下文
    LLMGeneration --> SQLValidation: 流式输出完成,解析出SQL+VizConfig
    SQLValidation --> SQLExecution: 校验通过(自动添加LIMIT)
    SQLValidation --> LLMGeneration: 校验失败_重试<=3
    SQLValidation --> Error: 重试超限
    SQLExecution --> ResultStreaming: 查询成功
    SQLExecution --> Error: 查询超时或异常
    ResultStreaming --> Completed: 数据+图表推送完成
    Completed --> [*]
    Error --> [*]: 返回错误事件
```
...
| **EChartsRenderer** | `EChartsRenderer.tsx` | 声明式 ECharts 渲染，透传 option，ResizeObserver 自适应；**支持用户手动切换三种图表类型（柱形图/饼状图/曲线图）**，AI 推荐默认类型，用户可通过顶部按钮自由切换，切换后根据原始数据实时重新渲染 |
...
stateDiagram-v2
    [*] --> Idle: 页面加载 / 点击Logo返回
    Idle --> Thinking: 用户发送问题
    Thinking --> ShowSQL: 收到event_sql
    ShowSQL --> ShowChart: 收到event_viz_config
    ShowChart --> Idle: 点击Logo返回首页
    Thinking --> ShowError: 收到event_error
    ShowSQL --> ShowError: 收到event_error
    ShowError --> Idle: 用户关闭错误 / 点击Logo

    state Thinking {
        [*] --> StreamDelta: event_thought(done=false)
        StreamDelta --> StreamDelta: 逐字追加
